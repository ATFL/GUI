...
DataLocation = '/home/pi/Desktop/Portable_mix/'
RawDataLocation = '/home/pi/Desktop/Portable_mix/'
...
train_data, val_data, test_data = preprocess_data(train_data, val_data, test_data, 3)
...
for i in range(len(SVMParamList)):
    clf = SVC(kernel=SVMParamList[i][2], C=SVMParamList[i][0], gamma=SVMParamList[i][1]).fit(train_data,train_targets[:,4])
    clf.fit(train_data, train_targets[:,4])
    acctrain = clf.score(train_data, train_targets[:,4])
    acctest = clf.score(val_data,val_targets[:,4])
    score = clf.score(test_data, test_targets[:,4])
    print(acctrain, acctest,score)

import pickle
filename = 'Port_Clf2.sav'
