...
def collect_data():
    ...
    while (time.time() < (start_time + duration_of_signal)):
        ...
        if (time.time() > (start_time + (sampling_time * sampling_time_index))):
            ...
            global dataVector
            global data_counter
            x1[data_counter] = mos.read()
            timeVector[data_counter] = time.time() - start_time
            data_counter += 1
            update_Graph()
    ...
    combinedVector = np.column_stack((timeVector,x1))
...
startB = start_Button()
linAc_exposeB = linAc_exposeButton()
linAc_recoverB = linAc_recoverButton()
