...
sensing_retract_time = 150 # time allowed before sensor is retracted, no longer exposed to sample
compressed_air_flush_time = 10 # time allowed for compressed air flushing of sensing chamber in seconds
duration_of_signal = 500 # time allowed for data acquisition per test run
...
def exposeAndCollectData():
    ...
    while (time.time() < (start_time + duration_of_signal)): # While time is less than duration of logged file
        ...
        if (time.time() > (start_time + (sampling_time * sampling_time_index))): # if time since last sample is more than the sampling time, take another sample
                    ...
                    global liveGraph
                    liveGraph.clear()
                    liveGraph.plot(timeVector, dataVector1, pen = 'm')
                    global app
                    app.processEvents() 
...
def inject_methane_and_ethane(methane_injection_conc, ethane_injection_conc):
    ...
    print ("Methane injection time is " + str(float(methane_injection_time)))
    print ("Ethane injection time is " + str(float(ethane_injection_time)))
