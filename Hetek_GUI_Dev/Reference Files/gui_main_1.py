...
class DataPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        runBtn = tk.Button(self, text='Run', command=lambda:combined_expose_collect())
        runBtn.place(relx=0.8,rely=0.9,relheight=0.1,relwidth=0.2)
...
def combined_expose_collect():
    ...
    while (time.time() < (start_time + duration_of_signal)) and (continueTest == True):  # While time is less than duration of logged file
        if (time.time() > (start_time + (
            ...
            xVal = (time.time() - start_time)
            yVal = mos.read()
            dataVector.append(yVal)  # Perform analog to digital function, reading voltage from first sensor channel
            timeVector.append(xVal)
