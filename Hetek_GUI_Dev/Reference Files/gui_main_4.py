...
class DataPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.runBtn = tk.Button(self, text='Run', command=lambda:start_purge_thread())
        self.runBtn.place(relx=0.8,rely=0.9,relheight=0.05,relwidth=0.2)

        stopBtn = tk.Button(self, text='Stop', command=lambda:end_testing())
        stopBtn.place(relx=0.8,rely=0.95,relheight=0.05,relwidth=0.2)

        statusFrame = tk.LabelFrame(self, text ='Status')
        statusFrame.place(relx=0.8,rely=0.4,relheight=0.5,relwidth=0.2)

        responseFrame = tk.Frame(self)
        responseFrame.place(relx=0.8,rely=0.6,relheight=0.3,relwidth=0.2)
...
class ManualControlPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.btn_1 = tk.Button(leftControlFrame, text='Extend Linear Actuator', command=lambda:linearActuator.extend())
        self.btn_1.place(relx=0,rely=0,relheight=0.45,relwidth=buttonWidth)
        self.btn_2 = tk.Button(leftControlFrame, text='Retract Linear Actuator', command=lambda:linearActuator.retract())
        self.btn_2.place(relx=0,rely=0.5,relheight=0.45,relwidth=buttonWidth)
        self.btn_3 = tk.Button(rightControlFrame, text='Read MOS', command=lambda:mos.print())
        self.btn_3.place(relx=0,rely=0,relheight=0.45,relwidth=buttonWidth)
        self.btn_4 = tk.Button(rightControlFrame, text='4', command=lambda:'none')
        self.btn_4.place(relx=0,rely=0.5,relheight=0.45,relwidth=buttonWidth)
...
def start_purge_thread():
    ...
    app.frames[DataPage].status.set('  Purgin\' chambers.')
...
def check_data_thread():
    if data_thread.is_alive():
        ...
        app.frames[DataPage].naturalGasLabel.config(bg='#FFC300')
        release_buttons()
